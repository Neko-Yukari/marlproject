"""Train ExplabOff on paper-accurate environment v2 — 10K episodes (optimized)."""
...
def train(M, E, episodes, log_interval=500):
    ...
    agents = [ExplabOffAgent(i, dim, adim, hidden_dim=64, lr=5e-5,  # smaller network
                             mi_mu=0.0, mi_nu=0.0, device=device)  # disable MI for speed
              for i in range(M)]
    hist = []
    t0 = time.time()
    ...
    for ep in range(episodes):
        ...
        for agent in agents:
            ...
            agent.update(batch_size=32, num_epochs=2)  # smaller batch, fewer epochs
...
print("="*60)
print("ExplabOff 10K | Paper-Accurate Env v2 | 2ES-3MD | Fast")
print("="*60)
results = {'ExplabOff_paper_10K': train(3, 2, 10000, 500)}

out = Path("results") / f"explaboff_paper_v2_fast_10k_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
out.mkdir(parents=True, exist_ok=True)
with open(out / "results.json", "w") as f:
    json.dump(results, f, indent=2)

with open(out / "explaboff_paper_v2_fast_10k.csv", "w") as f:
