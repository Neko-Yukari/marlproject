"""IPPO with Action Masking + Larger Network — 3ES-7MD Test."""
...
run_dir = f"results/ippo_masked_3es7md_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
reporter = TrainingReporter(run_dir, config={"M": M, "E": E, "algorithm": "IPPO+Mask", **STD})
...
for ep in range(EPISODES):
    ...
    for step in range(10):
        ...
        for i, agent in enumerate(agents):
            ...
            # Get action mask
            mask = None
            if STD.get("action_masking", False):
    ...
    if ep % LOG_INTERVAL == 0 or ep == EPISODES - 1:
        ...
        el = time.time() - t0
        print(f"\n{'='*60}")
        print(f"[IPPO+Mask] ep{ep:6d} cost={m['avg_cost']:.4f} comp={m['completion_rate']:.3f} "
